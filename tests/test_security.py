...
class TestSecurity:
    ...
    def test_no_credential_logging_in_auth_service(self):
        ...
        assert auth_service._verify_api_key(test_password, test_hash) is True

        # Verify incorrect password
        assert auth_service._verify_api_key("wrong", test_hash) is False
    ...
    async def test_smtp_credentials_not_logged(self):
        ...
        # Create settings with all required arguments
        settings = Settings(
            DEBUG=True,
            DATABASE_URL="postgresql://user:pass@host/db",
